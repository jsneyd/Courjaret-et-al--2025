%% FEM Code for the Less Tunnelling Calcium Model
...
% Load the Less Tunnelling Model's mesh data
load("Mesh_less_tunnel.mat")
% [ER_p,ER_e,ER_t], [Cyto_p,Cyto_e,Cyto_t], inter_bound_p

%% Specify the simulation time parameters
t_start = 0; %Start time
t_end = 1500; %End time
t_step = 0.01; %Simulation time step size
...
cortical_h_pointer = (Cyto_p(1,:) >= 1250 & Cyto_p(1,:) <= 1650 & Cyto_p(2,:) == 800);
cortical_h_p = round(Cyto_p(:,cortical_h_pointer),8);
cortical_h = zeros(num_t+1,size(cortical_h_p,2));
cortical_h(1,:) = 0.8;
cortical_h_init = cortical_h(1,:)';

%Initial conditions for h variable in the deep region (Deep Jipr 1)
deep_h_pointer = ((Cyto_p(1,:) >= 930 & Cyto_p(1,:) <= 2150 & Cyto_p(2,:) == 200) |...
    (Cyto_p(1,:) >= 930 & Cyto_p(1,:) <= 2150 & Cyto_p(2,:) == 400) |...
    (Cyto_p(1,:) == 2150 & Cyto_p(2,:) >= 200 & Cyto_p(2,:) <= 400));
...
%Initial ER Ca2+ in the ER-PM junction
cortical_ER_cal_pointer = (ER_p(1,:) >= 350 & ER_p(1,:) <= 550 & ...
    ER_p(2,:) >= 860 & ER_p(2,:) <= 885);
cortical_ER_cal_p = round(ER_p(:,cortical_ER_cal_pointer),8);
store_res_cortical_ER_cal = zeros(num_t+1,size(cortical_ER_cal_p,2));
init_cortical_ER_cal = init_val_ER_cal(cortical_ER_cal_p(3,:),:);
store_res_cortical_ER_cal(1,:) = init_cortical_ER_cal';
...
cortical_ER_cal_new = init_cortical_ER_cal;
...
for i = 1:num_t
    ...
    cortical_ER_cal_old = cortical_ER_cal_new;

    r_ER = RobinLoad2D(ER_p,ER_e,@kappa_val,@gD_val,@gN_val,...
        ER_cal_old,Cyto_cal_old,cortical_ER_cal_old,inter_bound_p,"ER_cal",...
        cortical_h_p,cortical_h_old,deep_h_p,deep_h_old,i,t_step);
    
    r_Cyto = RobinLoad2D(Cyto_p,Cyto_e,@kappa_val,@gD_val,@gN_val,...
        ER_cal_old,Cyto_cal_old,cortical_ER_cal_old,inter_bound_p,"Cyto_cal",...
        cortical_h_p,cortical_h_old,deep_h_p,deep_h_old,i,t_step);
    ...
    cortical_ER_cal_new = ER_cal_new(cortical_ER_cal_p(3,:),:);
    
    %Store the result
    store_res_ER_cal(i+1,:) = round(ER_cal_new',8);
    store_res_Cyto_cal(i+1,:) = round(Cyto_cal_new',8);
    store_res_cortical_ER_cal(i+1,:) = round(cortical_ER_cal_new',8);
...
save("Sim_TS_Less_Tunnelling.mat",'-v7.3')
...
%The initial condition function for c
...
function z = gN_val(x,y,loc2glb,...
            ER_cal,Cyto_cal,cortical_ER_cal,inter_bound_p,compartment, ...
    ...
    gamma = (1800200/2936)/(308400/1818);
    %Jserca parameters
    Vp = 240*1667.67; 
    K_bar_serca = 2.5e-7; 
    k_serca = 0.2;
    ...
    % Cortical SERCA flux
    if ((x(1) >= 350 && x(1) <= 800 && y(1) >= 770 && y(1) < 885) && ...
        (x(2) >= 350 && x(2) <= 800 && y(2) >= 770 && y(2) < 885)) || ...
      ((x(1) >= 350 && x(1) <= 400 && y(1) == 885) && ...
        (x(2) >= 350 && x(2) <= 400 && y(2) == 885)) || ...
       ((x(1) >= 500 && x(1) <= 550 && y(1) == 885) && ...
        (x(2) >= 500 && x(2) <= 550 && y(2) == 885))
    ...
    elseif ((x(1) >= 930 && x(1) <= 2150 && y(1) == 200) && ...
            (x(2) >= 930 && x(2) <= 2150 && y(2) == 200)) || ...
           ((x(1) >= 930 && x(1) <= 2150 && y(1) == 400) && ...
            (x(2) >= 930 && x(2) <= 2150 && y(2) == 400)) || ...
           ((x(1) == 2150 && y(1) >= 200 && y(1) <= 400) && ...
            (x(2) == 2150 && y(2) >= 200 && y(2) <= 400))
    ...
    % Cortical IP3R flux
    elseif ((x(1) >= 1250 && x(1) <= 1650 && y(1) == 800) && ...
            (x(2) >= 1250 && x(2) <= 1650 && y(2) == 800))
        [loc_ER_cal,loc_Cyto_cal] = loc_cal_func(inter_bound_p,loc2glb,ER_cal,Cyto_cal,compartment);
    ...
    elseif ((x(1) >= 0 && x(1) <= 2350 && y(1) == 900) && ...
        ...
        Jpm = Jano+Vpm*loc_Cyto_cal^2/(Kpm^2+loc_Cyto_cal^2);
        
        % SOCE location
        if ((x(1) >= 400) && (x(1) <= 500) && (y(1) == 900) && ...
            (x(2) >= 400) && (x(2) <= 500) && (y(2) == 900))
                
            STIM = K_stim^STIM_Hill/(K_stim^STIM_Hill+mean(cortical_ER_cal,"all")^STIM_Hill);
